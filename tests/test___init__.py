...
#                           --- Module Imports ---                            #
# ----------------------------------------------------------------------------#
from topshelfsoftware_logging import (  # noqa: E402
    get_logger,
    add_log_stream,
...
@pytest.mark.parametrize(
    "event_file", get_json_files(MODULE_EVENTS_DIR, ["get_logger", "success"])
)
...
@pytest.mark.parametrize(
    "event_file",
    get_json_files(MODULE_EVENTS_DIR, ["add_log_stream", "success"]),
)
...
@pytest.mark.parametrize(
    "event_file",
    get_json_files(MODULE_EVENTS_DIR, ["create_console_handler", "success"]),
)
...
@pytest.mark.parametrize(
    "event_file",
    get_json_files(MODULE_EVENTS_DIR, ["clear_root_handlers", "success"]),
)
def test_04_clear_root_handlers(get_event_as_dict):
    print_section_break()
    logger.info(f"Test Description: {get_event_as_dict['description']}")
    clear_root_handlers()
